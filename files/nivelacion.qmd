...
Tiramos dos monedas simultáneamente: una justa y una cargada (90% probabilidad de cara). Si los lanzamientos son independientes, $\mathbb{P}(\text{Cara}_1\cap\text{Cara}_2) = 0.5 \times 0.9 = 0.45$. Lo verificamos simulando:

En el código, `rbinom(n, 1, prob = p)` genera $n$ resultados codificados como $1$ (cara) o $0$ (sello), con probabilidad $p$ de obtener $1$. `set.seed(1)` permite reproducir exactamente la misma simulación.
