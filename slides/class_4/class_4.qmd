...
ditrib_X <- table(X) %>% as_tibble() %>% mutate(p=round(n/sum(n), 3)) %>% select(-n)
...
table(X) %>% 
  ...
  theme(legend.position="none",
        panel.border = element_blank()) # Esta línea agrega el recuadro interior
...
# Crear un data frame para graficar la CDF
x_values <- 0:n  # Possible outcomes range from 0 to n
...
# Filtrar valores de 0 a 5 para la superposición
