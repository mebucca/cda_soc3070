...
# datos del plebiscito chileno de 1988, del paquete "carData"
library(pacman)
p_load("carData")
p_load("viridis")
p_load("tidyverse")
...
- $\text{logit}(p_{ij})$ es el [log odds]{.bold} de votar N/U/Y vs A.
...
[Importante:]{.bold} los coeficientes y sus transformaciones entregan información sobre las probabilidades [relativas]{.bold} de los diferentes $j$'s (con respecto a categoría de referencia).
...
Analizando $$\frac{\partial p_{ij}}{\partial x_{k}} = p_{ij} \cdot \bigg(\beta_{jk} - \sum^{J-1}_{l=1}p_{il} \cdot \beta_{lk}\bigg)$$
...
podemos notar que  $\sum^{J-1}_{l=1}p_{il} \cdot \beta_{lk} \equiv \overline{\beta_{k}}, \quad$ el "efecto promedio de" $x_k$ (no depende de $j$). Se desprende que:

:::

. . .

- El signo del efecto marginal de los predictores [no necesariamente]{.bold} corresponde al signo del parámetro estimado en la regresión.

. . .

  - $\frac{\partial p_{ij}}{\partial x_{k}} > 0 \quad$  si  $\quad \beta_{jk} > \sum^{J-1}_{l=1}p_{il} \cdot \beta_{lk}$
  - $\frac{\partial p_{ij}}{\partial x_{k}} = 0 \quad$  si  $\quad \beta_{jk} = \sum^{J-1}_{l=1}p_{il} \cdot \beta_{lk}$
  - $\frac{\partial p_{ij}}{\partial x_{k}} < 0 \quad$  si  $\quad \beta_{jk} < \sum^{J-1}_{l=1}p_{il} \cdot \beta_{lk}$

. . .

[Nota]{.bold}: No tiene mucho sentido testear si efectos marginales son distintos de cero.

## Efectos marginales sobre la probabilidad de la categoría $j$

::: {.pull-left}

si $$\frac{\partial p_{ij}}{\partial x_{k}} = p_{ij} \cdot \bigg(\beta_{jk} - \sum^{J-1}_{l=1}p_{il} \cdot \beta_{lk}\bigg)$$
...
y $$p_{ij} =\frac{e^{\beta_{j0} + \beta_{j1}x_{1i} + \dots + \beta_{jk}x_{ki}}}{1 + \sum^{J-1}_{l=1}  e^{\beta_{l0} + \beta_{l1}x_{1i} + \dots + \beta_{lk}x_{ki}}}$$
...
::: {.pull-left}
  ...
  $$p_{ij} =\frac{e^{\beta_{j0} + \beta_{j1}\text{statusquo}_{i} + \beta_{j2}\text{male}_{i}}}{1 + \sum^{J-1}_{l=1} e^{\beta_{l0} + \beta_{l1}\text{statusquo}_{i} + \beta_{l2}\text{male}_{i}}}$$
...
- El efecto marginal de $x_{k}$ sobre $\text{logit}(p_{ij})$ es constante e igual a $\beta_{jk}$; exponenciando, $e^{\beta_{jk}}$ es la odds ratio de $j$ vs $J$ asociada a un aumento de una unidad en $x_{k}$.
- El efecto marginal sobre la **probabilidad** $p_{ij}$ no es $\beta_{jk}$: $\partial p_{ij}/\partial x_{k} = p_{ij}(\beta_{jk} - \overline{\beta_{k}})$, por lo que su signo y magnitud dependen de $p_{ij}$ y pueden variar entre observaciones (e incluso cambiar de signo).
- En la práctica se resume esta heterogeneidad con AME, MEM o MER (ej., `avg_slopes()` del paquete `marginaleffects`).
- El modelo logit multinomial supone Independencia de Alternativas Irrelevantes (IIA); no existe un test concluyente para verificarlo.
- La próxima clase aborda la regresión de Poisson, usada para modelar variables de conteo.
