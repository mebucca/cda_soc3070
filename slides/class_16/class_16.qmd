...
```{r}
baseline_train_fit <- glm.fit(
  x = cbind(`(Intercept)` = 1, x_train), y = y_train,
  family = binomial()
)
baseline_valid_prob <- as.numeric(plogis(
  cbind(`(Intercept)` = 1, x_valid) %*% coef(baseline_train_fit)
))
baseline_threshold <- map_dfr(thresholds, function(t) {
  class_metrics(y_valid, baseline_valid_prob, t) %>% mutate(threshold = t)
}) %>%
  slice_min(cost_per_100, n = 1, with_ties = FALSE) %>%
  pull(threshold)
...
tibble(
  ...
  acción = if_else(
    test_prob[case_id] >= selected_threshold,
    "priorizar",
    "no priorizar"
  ),
