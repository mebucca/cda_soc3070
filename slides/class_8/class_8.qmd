...
[Calculemos ahora un IC al 95% para el efecto multiplicativo de la "temperatura" sobre las odds de alta demanda:]{.bold} $e^{\beta_{2}}$= `r expbeta2=exp(beta2); expbeta2`
...
- Los estimadores MLE de un GLM (como la regresión logística) son consistentes, insesgados y asintóticamente normales — de ahí que $\hat\theta \pm z \cdot SE_{\hat\theta}$ sea un intervalo de confianza válido en muestras grandes.
- Test de Wald: divide el coeficiente por su error estándar y lo compara con una Normal; usa la estimación de **un solo** modelo.
- Test de razón de verosimilitudes (LR): compara la log-verosimilitud de dos modelos anidados; suele ser más confiable que Wald con muestras chicas o coeficientes grandes.
- Bootstrap: remuestrea los datos con reemplazo para aproximar la distribución muestral de cualquier estimate (SE, CI) sin depender de fórmulas asintóticas.
- La próxima clase extiende el modelo logístico a variables dependientes con más de dos categorías no ordenadas: regresión logística multinomial.
