...
Usaremos la base `bikedata` para predecir si un hora tiene alta demanda (`high_demand = 1`). Consideramos dos predictores:

* **temp**: temperatura normalizada.  
* **hum**: humedad normalizada.

```{r}
source("../_shared/bikeshare.R"); course <- use_course_data("inter")
...
Queremos estudiar la **distribución bootstrap de las predicciones de probabilidad** de alta demanda para varios valores de `temp` (temperatura), manteniendo fija la humedad en su media. Esto nos permitirá construir intervalos de confianza para toda una curva de predicción.

```{r}
set.seed(123)

# Valores de temp de interés
wt_vals <- seq(min(bikedata$temp), max(bikedata$temp), length.out = 20)
...
bs_preds <- function(split){
  ...
  m <- glm(high_demand ~ temp + hum, data = d, family = binomial(link = "logit"))
  newdata <- tibble(temp = wt_vals, hum = mean(bikedata$hum))
  tibble(temp = wt_vals, pred = predict(m, newdata = newdata, type = "response"))
}

# Generar 200 muestras bootstrap
boots <- bootstraps(bikedata, times = 200, apparent = TRUE)
...
- `bootstraps(bikedata, times = 200, apparent = TRUE)` genera 200 muestras bootstrap y una réplica "aparente" con los datos originales.  
- En cada muestra, la función `bs_preds` ajusta el modelo logístico y calcula predicciones en una grilla de valores de `temp`.  
...
- Vemos cómo la incertidumbre es mayor en los extremos de la distribución de `temp`, donde hay menos datos disponibles.
...
ggplot(sample_curves, aes(x = temp, y = pred, group = id)) +
  geom_line(alpha = 0.2, color = julia$navy) +
  labs(
    title = "Curvas de predicción bootstrap de P(high_demand=1)",
