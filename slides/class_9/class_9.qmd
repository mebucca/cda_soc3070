...
subtitle: "Regresión Logística Multinomial - Estructura Teórica"
author: "Mauricio Bucca · Sociología UC"
date: today
body-classes: theme-ajax
course-team: ajax
...
interpretación de efectos multinomiales.
:::
...
# Regresión Logística Multinomial {.inverse .center .middle}

## Estructura de un modelo de regresión logística multinomial
Un modelo de regresión logística multinomial generaliza la regresión logística (binomial) a situaciones en que la variable dependiente es una [variable discreta con dos o más valores no ordenados]{.bold} (ejemplo: voto entre tres candidatos, elección de barrio, etc).
...
- Para cada observación observamos datos $y_{i}, \dots , y_{n}$ que actúan como variable dependiente, donde $y_{i} \in \{1,2, \cdots, J\}$
  - Las $J$ categorías de $y_{i}$ no siguen necesariamente un orden.

. . .

- Asumimos que estos datos son realizaciones de $n$ variables aleatorias que siguen una distribución Multinomial con probabilidades desconocidas: $Y_{i} \sim \text{Multinomial}(\boldsymbol{p_{i}}: p_{1}, \dots, p_{J})$

. . .

- Dichas probabilidades, $\boldsymbol{p_{i}}$, varían de individuo en individuo en función de ciertas covariables.

## Distribución Multinomial

[Distribución Multinomial:]{.bold}
...
[Ejemplo]{.bold}: Al tirar un dado justo 12 veces, ¿cuál es la probabilidad de obtener cada lado 2 veces?
```{r}
dmultinom(x=c(2,2,2,2,2,2), size=12 ,p=c(1/6,1/6,1/6,1/6,1/6,1/6))
```

## Distribución Multinomial (n=1)
[Distribución Multinoulli o Categórica:]{.bold}
...
[Ejemplo]{.bold}: Al tirar un dado justo 1 vez, ¿cuál es la probabilidad de obtener el número 4?
```{r}
...
- Específicamente, queremos estimar ( $p_{ij}$ ) como una función de covariables, usando número de parámetros $k$, tal que $k<n$.
...
$$p_{ji} =\frac{e^{\beta_{j0} + \beta_{j1}x_{1i} + \dots + \beta_{jk}x_{ki}}}{1 + \sum^{J-1}_{l=1}  e^{\beta_{l0} + \beta_{l1}x_{1i} + \dots + \beta_{lk}x_{ki}}}$$
...
## Regresión logística multinomial en la práctica {.inverse .center .middle}
...
# datos del plebiscito de 1988 (paquete "carData")
library(pacman)
p_load("carData")
p_load("viridis")
p_load("tidyverse")
...
\end{bmatrix}
\begin{bmatrix}
\beta_{01} & \beta_{02} & \dots & \beta_{0(J-1)} \\
\beta_{11} & \beta_{12} & \dots & \beta_{1(J-1)} \\
\vdots & \vdots & \ddots & \vdots \\
\beta_{p1} & \beta_{p2} & \dots & \beta_{p(J-1)}
\end{bmatrix}
$$
...
predictions %>% filter(str_detect(quant, "^l")) %>% separate(quant,sep=8, into=c("quant","vote")) %>%
  ...
  labs(x="Apoyo al status-quo", y="Logit(Vote=j)", colour="vote")
```
...
predictions %>% filter(str_detect(quant, "^p")) %>%  separate(quant,sep=2, into=c("quant","vote")) %>%
  ...
  axis.title.y = element_text(size = 24), axis.title.x = element_text(size = 24),
  legend.text = element_text(size = 18), legend.position="top") +
  labs(x="Apoyo al status-quo", y="P(Vote=j)", colour="vote")
```
...
- La regresión logística multinomial generaliza el modelo binomial cuando $y_{i}$ toma $J$ valores no ordenados: se estiman $J-1$ logits, cada uno comparando una categoría $j$ contra una categoría de referencia $J$.
- Cada $y_{i}$ es una realización de una distribución Multinomial (o Multinoulli si $n=1$) con probabilidades $p_{i1}, \dots, p_{iJ}$.
- De los $J-1$ logits se despeja $p_{ji}$ como una función softmax de los predictores lineales: $p_{ji} = \dfrac{e^{\beta_{j0} + \beta_{j1}x_{1i} + \dots + \beta_{jk}x_{ki}}}{1 + \sum^{J-1}_{l=1} e^{\beta_{l0} + \beta_{l1}x_{1i} + \dots + \beta_{lk}x_{ki}}}$.
- Los parámetros se estiman via MLE, imponiendo la restricción $\sum_{j}p_{ij} = 1$ (optimización simultánea, no $J-1$ regresiones separadas).
- La próxima clase aborda la interpretación de estos coeficientes: efectos sobre el logit, odds ratios, y efectos marginales sobre las probabilidades.
