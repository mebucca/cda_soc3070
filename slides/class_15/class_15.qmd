...
# Reutiliza logit_demand, bikedata y p_hat ya calculados más arriba
# (mismo modelo y mismos datos: no hace falta reajustar nada aquí)

# ROC
roc_obj <- roc(response = bikedata$high_demand, predictor = p_hat, quiet = TRUE)
...
calc_rates <- function(tau) {
  ...
  TP <- sum(yhat == 1 & bikedata$high_demand == 1)
  TN <- sum(yhat == 0 & bikedata$high_demand == 0)
  FP <- sum(yhat == 1 & bikedata$high_demand == 0)
  FN <- sum(yhat == 0 & bikedata$high_demand == 1)
...
ggplot(bikedata, aes(x = p_hat, fill = factor(high_demand))) +
...
set.seed(123)
random_scores <- runif(nrow(bikedata))
roc_random <- roc(bikedata$high_demand, random_scores, quiet = TRUE)
...
El modelo detrás ya lo conocemos: la regresión logística **multinomial** de la clase 9, ahora evaluada como clasificador en vez de interpretada como modelo explicativo.
...
- Log-loss (clase 3), en cambio, se generaliza de forma inmediata al caso multinomial — es la misma cross-entropy que usamos para ajustar `logit_multi`, y sigue siendo una sola cifra comparable entre modelos.

## En resumen

- La regresión logística es también un clasificador de ML: ajusta $X\beta$ maximizando la verosimilitud, equivalente a minimizar el log-loss.
- Un umbral $\tau$ convierte probabilidades en clases; su elección depende del costo relativo de cada tipo de error, no de una convención fija.
- La matriz de confusión resume aciertos y errores; sensibilidad, especificidad, precisión y AUC capturan aspectos distintos del desempeño.
- La curva ROC recorre todos los umbrales posibles; el AUC resume ese trade-off en un solo número.
- El caso multinomial generaliza estas ideas: matriz $J\times J$, métricas one-vs-rest, y un log-loss que se extiende sin cambios.
- La próxima clase pregunta qué hacer cuando ya no confiamos en la forma funcional elegida a mano: regularización con Ridge y LASSO.
