...
set.seed(3070)
days <- sort(unique(bikedata$day))
n_train_days <- floor(0.60 * length(days))
n_valid_days <- floor(0.20 * length(days))
...
id_train <- which(bikedata$day %in% train_days)
id_valid <- which(bikedata$day %in% valid_days)
id_test <- which(bikedata$day %in% test_days)
...
La clase 19 terminó con una pregunta sin responder: regularización controla **cuánta** complejidad usar, pero seguimos siendo nosotros quienes escribimos a mano qué interacciones probar (`I(temp^2)`, `factor(hr)*workingday`...). Cuando agregamos la interacción completa hora×mes, no sabíamos de antemano cuáles de esas variables eran señal y cuáles ruido.
...
tree_class_hat <- factor(if_else(tree_prob >= 0.5, "alta", "baja"),
                         levels = c("baja", "alta"))
...
```{r}
tibble(
  accuracy = mean(tree_class_hat == test$high_demand),
  sensitivity = mean(tree_class_hat[test$high_demand == "alta"] == "alta"),
  specificity = mean(tree_class_hat[test$high_demand == "baja"] == "baja"),
  log_loss = log_loss(test$high_demand, tree_prob)
)
```

Las probabilidades de un árbol son constantes dentro de cada hoja.
...
best_mtry <- rf_grid %>% slice_min(validation_loss, n = 1) %>% pull(mtry)
rf_final <- randomForest(
  class_formula, data = dev, ntree = 350, mtry = best_mtry,
  nodesize = 15, importance = TRUE
)
rf_prob <- predict(rf_final, test, type = "prob")[, "alta"]
rf_class <- factor(if_else(rf_prob >= 0.5, "alta", "baja"),
                   levels = c("baja", "alta"))

tibble(
  modelo = c("Árbol", "Random Forest"),
  accuracy = c(mean(tree_class_hat == test$high_demand),
               mean(rf_class == test$high_demand)),
  log_loss = c(log_loss(test$high_demand, tree_prob),
               log_loss(test$high_demand, rf_prob))
)
```
...
Con pérdida cuadrática $L=\tfrac12\sum_i (y_i-f(x_i))^2$:

$$
-\frac{\partial L}{\partial f(x_i)}=y_i-f(x_i)=r_i.
$$

- Cada árbol se ajusta a los **residuos** del ensamble actual.
- Con pérdida de Poisson (la que usamos para `bikers`) el gradiente es $y_i-e^{f(x_i)}$: mismo principio, distinta pérdida.
...
boost_grid <- crossing(depth = c(1, 2, 3), shrinkage = c(0.03, 0.08)) %>%
  mutate(
    model = map2(depth, shrinkage, ~ gbm(
      reg_formula, data = train, distribution = "poisson",
      n.trees = 260, interaction.depth = .x, shrinkage = .y,
      n.minobsinnode = 20, bag.fraction = 0.7, verbose = FALSE
    )),
    prediction = map(model, ~ predict(.x, valid, n.trees = 260, type = "response")),
    validation_rmse = map_dbl(prediction, ~ rmse(valid$bikers, .x))
  )

boost_grid %>% select(depth, shrinkage, validation_rmse) %>% arrange(validation_rmse)
```

## Gradient Boosting: ejemplo empírico

```{r boost-final, cache=TRUE}
best_boost <- boost_grid %>% slice_min(validation_rmse, n = 1)
boost_final <- gbm(
  reg_formula, data = dev, distribution = "poisson",
  n.trees = 260,
  interaction.depth = best_boost$depth,
  shrinkage = best_boost$shrinkage,
  n.minobsinnode = 20, bag.fraction = 0.7, verbose = FALSE
)
boost_pred <- predict(boost_final, test, n.trees = 260, type = "response")
poisson_baseline <- glm(reg_formula, data = dev, family = poisson())
poisson_pred <- predict(poisson_baseline, test, type = "response")

tibble(
  modelo = c("Poisson", "Árbol podado", "Gradient Boosting"),
  RMSE = c(rmse(test$bikers, poisson_pred),
           rmse(test$bikers, tree_reg_pred), rmse(test$bikers, boost_pred)),
  MAE = c(mae(test$bikers, poisson_pred),
          mae(test$bikers, tree_reg_pred), mae(test$bikers, boost_pred))
)
```
...
tibble(observed = test$bikers, predicted = boost_pred) %>%
  slice_sample(n = min(1400, nrow(test))) %>%
...
tibble(predicted = boost_pred, residual = test$bikers - boost_pred) %>%
  ggplot(aes(predicted, residual)) +
  geom_hline(yintercept = 0, color = course_palette[["secondary"]]) +
  geom_point(color = course_palette[["primary"]], alpha = 0.3) +
  geom_smooth(se = FALSE) +
  labs(x = "Predicción", y = "Residuo")
```

Patrones persistentes indican estructura que el ensamble todavía no captura.
