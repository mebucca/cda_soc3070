...
## Función Media (inverso de función enlace)
...
preds <- tibble(x = c(0, 1)) %>% mutate(mu = eta(x), sd = sdy(x))
mu0 <- preds$mu[1]; sd0 <- preds$sd[1]
mu1 <- preds$mu[2]; sd1 <- preds$sd[2]
ratio <- mu1 / mu0

# Rango de x adaptado al ancho real de ambas normales, no fijo.
x_lo <- max(0, min(mu0 - 4 * sd0, mu1 - 4 * sd1))
x_hi <- max(mu0 + 4 * sd0, mu1 + 4 * sd1)
y_top <- max(dnorm(mu0, mu0, sd0), dnorm(mu1, mu1, sd1))

dens <- tibble(x = seq(x_lo, x_hi, length.out = 500)) %>%
  mutate(`no hábil` = dnorm(x, mu0, sd0), `hábil` = dnorm(x, mu1, sd1)) %>%
  pivot_longer(-x, names_to = "day_type", values_to = "density") %>%
  mutate(day_type = factor(day_type, levels = c("no hábil", "hábil")))

plot <- ggplot(dens, aes(x = x, y = density, fill = day_type, colour = day_type)) +
  geom_area(position = "identity", alpha = 0.05, linewidth = 1.2) +
  geom_vline(xintercept = mu0, colour = course_palette[["primary"]],
             linetype = "dashed", linewidth = 0.9) +
  geom_vline(xintercept = mu1, colour = course_palette[["secondary"]],
             linetype = "dashed", linewidth = 0.9) +
  annotate("text", x = mu0, y = y_top * 1.08, label = paste0("μ = ", round(mu0)),
           colour = course_palette[["primary"]], size = 6.5, fontface = "bold") +
  annotate("text", x = mu1, y = y_top * 1.18, label = paste0("μ = ", round(mu1)),
           colour = course_palette[["secondary"]], size = 6.5, fontface = "bold") +
  annotate("text", x = (mu0 + mu1) / 2, y = y_top * 1.32,
           label = paste0(round(ratio, 2), "x más arriendos en días hábiles"),
           colour = "black", size = 5.5, fontface = "italic") +
  scale_x_continuous(name = "Arriendos por hora") +
  scale_y_continuous(name = "densidad", expand = expansion(mult = c(0, 0.05))) +
  coord_cartesian(ylim = c(0, y_top * 1.4), clip = "off") +
  scale_fill_manual(values = c("no hábil" = course_palette[["primary"]],
                                "hábil" = course_palette[["secondary"]])) +
  scale_colour_manual(values = c("no hábil" = course_palette[["primary"]],
                                 "hábil" = course_palette[["secondary"]])) +
